<template>
	...
	>
		<AppLogo :showTitle="false" :class="`${prefixCls}-logo`" :imgSrc="getLogoImgSrc" :imgStyle="getLogoImgStyle" />
		...
		<ScrollContainer>
			<ul :class="`${prefixCls}-module`">
				...
				>
					...
					<Icon v-else :class="`${prefixCls}-module__icon`" :size="getCollapsed ? 16 : 20" :icon="item.icon || (item.meta && item.meta.icon)" />
...
const go = useGo()
const { getMenuWidth, getCanDrag, getCloseMixSidebarOnChange, getMenuTheme, getMixSideTrigger, getRealWidth, getMixSideFixed, mixSideHasChildren, setMenuSetting, getIsMixSidebar, getCollapsed } = useMenuSetting()
...
const getLogoImgSrc = computed(() => (unref(getCollapsed) ? collapsedLogoImg : expandedLogoImg))
