...
import { ColorEnum, DicCodeEnum, DicCommonPrivateEnum, DicShowHideEnum, DicSortEnum, DicStatusEnum, DicYesNoEnum } from '@/enums'
...
/** 字典查询 */
export const dictMap = await dicDictList(['auth_frame_type', DicCodeEnum.SYS_YES_NO, DicCodeEnum.SYS_SHOW_HIDE, 'auth_menu_type', DicCodeEnum.SYS_NORMAL_DISABLE, 'sys_common_private'])
...
export const isExternalLinksMenu = (menuType: string, frameType: string) => isMenu(menuType) && frameType == FrameTypeEnum.EXTERNAL_LINKS
...
export const columns: BasicColumn[] = [
	...
	},
	{
		...
		customRender: ({ record }) => {
			const data = record as MenuIM
			return data.isCommon === DicCommonPrivateEnum.PRIVATE ? h(Tag, { color: ColorEnum.ORANGE }, () => data?.enterpriseInfo?.nick || '-') : h(Tag, { color: ColorEnum.BLUE }, () => '公共')
		},
	},
]
...
export const formSchema: FormSchema[] = [
	...
	},
	{
		...
		required: ({ values }) => !(isExternalLinksMenu(values.menuType, values.frameType) || isButton(values.menuType)),
		ifShow: ({ values }) => !(isExternalLinksMenu(values.menuType, values.frameType) || isButton(values.menuType)),
		colProps: { span: 12 },
	},
	{
		label: '菜单图标',
	...
	},
	{
		...
		ifShow: ({ values }) => isEmbeddedMenu(values.menuType, values.frameType) || isExternalLinksMenu(values.menuType, values.frameType),
		dynamicRules: ({ values }) =>
			isEmbeddedMenu(values.menuType, values.frameType) || isExternalLinksMenu(values.menuType, values.frameType)
				? [
						{
							required: true,
							message: '请输入外链路径',
	...
	},
	{
		...
		required: ({ values }) => isDetails(values.menuType) || isNormalMenu(values.menuType, values.frameType),
		ifShow: ({ values }) => isDetails(values.menuType) || isNormalMenu(values.menuType, values.frameType),
		colProps: { span: 12 },
	},
	{
		label: '路由参数',
	...
	},
	{
		...
		required: ({ values }) => !isDir(values.menuType) && !isExternalLinksMenu(values.menuType, values.frameType),
		ifShow: ({ values }) => !isDir(values.menuType) && !isExternalLinksMenu(values.menuType, values.frameType),
		colProps: { span: 12 },
	},
	{
		label: '页面缓存',
		...
		required: ({ values }) => (isMenu(values.menuType) || isDetails(values.menuType)) && !isExternalLinksMenu(values.menuType, values.frameType),
		ifShow: ({ values }) => (isMenu(values.menuType) || isDetails(values.menuType)) && !isExternalLinksMenu(values.menuType, values.frameType),
		colProps: { span: 12 },
	},
	{
		label: '固定标签',
