<template>
	<BasicModal v-bind="$attrs" @register="registerModal" :title="getTitle" :defaultFullscreen="true" :showCancelBtn="false" :showOkBtn="current === 2" @ok="handleSubmit">
			...
			<div class="mt-5">
				...
				<BasicForm @register="authRegister" v-show="current === roleInitList[1].current">
					<template #auth="{ model, field }">
						<BasicTree v-model:value="model[field]" :treeData="authTree" :fieldNames="{ title: 'label', key: 'id' }" checkable toolbar @check="authCheck" title="功能权限" />
					</template>
				</BasicForm>
				<BasicForm @register="organizeRegister" v-show="current === roleInitList[2].current">
					<template #organize="{ model, field }">
						<BasicTree v-model:value="model[field]" :treeData="organizeTree" :fieldNames="{ title: 'label', key: 'id' }" checkable toolbar @check="dataScopeCheck" title="数据权限" />
					</template>
				</BasicForm>
			</div>
		</div>
...
import { authFormSchema, getOrgDeptIds, organizeFormSchema, roleFormSchema, roleInitList } from './role.data'
...
const [authRegister, { setFieldsValue: authSetFieldsValue, resetFields: authResetFields, validate: authValidate }] = useForm({
	labelWidth: 100,
	schemas: authFormSchema,
	showActionButtonGroup: false,
})

const [organizeRegister, { setFieldsValue: organizeSetFieldsValue, resetFields: organizeResetFields, validate: organizeValidate }] = useForm({
	labelWidth: 100,
...
async function handleSubmit() {
	try {
		const [role, auth, organize] = await Promise.all([roleValidate(), authValidate(), organizeValidate()])
...
function authCheck(checkedKeys, e) {
	...
	auth.authHalfKeys = e.halfCheckedKeys
}

/** 获取组织Id */
function dataScopeCheck(checkedKeys) {
